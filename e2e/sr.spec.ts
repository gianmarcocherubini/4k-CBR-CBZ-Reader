...
test('Anime4K super resolution: enhanced canvas, badge, level probe, faithful output (or clean fallback)', async ({ page }) => {
  ...
  await expect(enhanced).toHaveAttribute('data-sr-width', '1600') // 800 px source -> 2x
  ...
  // Faithfulness: the 2x output downsampled back must match the source closely (no strip
