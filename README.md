...
- **Made for manga, and for webtoons.** Right-to-left by default, smart double pages (wide spreads and the cover stay alone), a *blank page here* fix when a volume's pairs are misaligned, an adjustable gutter, pinch and double-tap zoom, four fit modes, page-turn transitions, full screen while reading. Or switch to **vertical scrolling**: one continuous strip with native momentum, three widths and an optional gap between pages, the bookmark kept by position.
